...
use gpui::{
    ...
    MouseUpEvent, Pixels, Point, ScrollHandle, Styled, Window, canvas, point, px, quad, rgb, size,
};
    ...
    S: Scrollable + Clone + 'static,
{
    ...
    canvas(
        |_, _, _| (),
        move |track: Bounds<Pixels>, _, window: &mut Window, _cx: &mut App| {
            ...
            if opacity > 0.0 {
                window.paint_quad(quad(
                    ...
                    if active {
                        ...
                        rgb(dark::TEXT_GHOST).opacity(0.55)
                    }
                    .opacity(opacity),
                    px(0.0),
