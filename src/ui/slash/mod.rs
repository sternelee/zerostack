...
pub async fn handle_slash(
    ...
    match parts[0] {
        ...
        cmd if cmd == "/ponytail" || cmd.starts_with("/ponytail ") || cmd == "/ponytail-review" => {
            let arg = if parts.len() > 1 { parts[1] } else { "" };
            match arg {
                "" => {
                    // Show current mode.
                    match &ctx.context.ponytail_mode {
                        Some(m) => write_ok(ctx.renderer, format!("Ponytail mode: {m}")),
                        None => write_result(
                            ctx.renderer,
                            "Ponytail mode: off. Use /ponytail lite|full|ultra to activate.",
                        ),
                    }
                }
                "lite" | "full" | "ultra" => {
                    ctx.context.ponytail_mode = Some(arg.to_string());
                    // Load ponytail skill as current prompt.
                    if let Some(skill) = ctx.context.skills.get("ponytail") {
                        ctx.context.current_prompt = Some(skill.content.clone());
                        ctx.context.current_prompt_name = Some(format!("ponytail-{arg}"));
                    }
                    write_ok(
                        ctx.renderer,
                        format!("Ponytail mode: {arg}. Use /ponytail off to disable."),
                    );
                }
                "off" => {
                    ctx.context.ponytail_mode = None;
                    ctx.context.current_prompt = None;
                    ctx.context.current_prompt_name = None;
                    write_ok(ctx.renderer, "Ponytail mode: off. Back to normal coding.");
                }
                "review" | _ if cmd == "/ponytail-review" => {
                    // Review the last change through ponytail lens.
                    let mode = ctx
                        .context
                        .ponytail_mode
                        .clone()
                        .unwrap_or_else(|| "full".into());
                    write_ok(ctx.renderer, format!("Ponytail review ({mode} mode):"));
                    write_result(ctx.renderer, "Review the last change. Ask:");
                    write_result(
                        ctx.renderer,
