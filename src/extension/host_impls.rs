...
use crate::extension::host::zerostack::extension::{
    command_registry as wit_cmd, exec as wit_exec, external_dirs as wit_ed,
    file_mutation_queue as wit_fmq, http as wit_http, provider_registry as wit_pr,
    session_control as wit_sc, tool_registry as wit_tr, trigger_prompt as wit_tp,
    truncator as wit_trunc, ui_prompt as wit_uip, ui_status as wit_uis,
};
...
impl wit_ed::Host for ExtGuestState {
    fn add_dir(&mut self, path: String) -> Result<(), __internal_string> {
    ...
    fn remove_dir(&mut self, path: String) -> Result<(), __internal_string> {
        let resolved = resolve_external_dir(&self.host_context.cwd, &path)
    ...
    fn list_dirs(&mut self) -> Vec<String> {
        self.external_dirs.clone()
    }

    fn has_dir(&mut self, path: String) -> bool {
